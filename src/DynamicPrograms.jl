...
function interpolate_policy(
    ...
    return RegularGridBsplineList(reshape(splines,size(u)[1]))
end 
...
function DynamicProgram(
    ...
    DP = DynamicProgram(R,F,p,u,X,δ,V,P)
    if solve == true
...
"""
function estimate_time(DP::DynamicProgram)
